	...
	(
		...
		(
			...
			(
				...
				'icon'                => 'system/modules/chronik/assets/images/club_added.gif',
				'button_callback'     => array('tl_chronik', 'clubIcon')
			),
			'players' => array
			(
				'label'               => &$GLOBALS['TL_LANG']['tl_chronik']['players'],
				'icon'                => 'system/modules/chronik/assets/images/player_added.png',
...
class tl_chronik extends Backend
{
	...
	public function clubIcon($row, $href, $label, $title, $icon, $attributes)
	{
		...
		{
			$icon = 'system/modules/chronik/assets/images/club_red.gif';
	...
	public function playerIcon($row, $href, $label, $title, $icon, $attributes)
	{
		...
		{
			$icon = 'system/modules/chronik/assets/images/player_red.png';
	...
	{
		return \Samson\Helper::getDateString($group);
	}  
	...
	public function getSpieler(DataContainer $dc)
	{
		...
		while($objSpielerregister->next())
		{
			...
			$name .= $objSpielerregister->birthday ? '* '.\Samson\Helper::getDate($objSpielerregister->birthday) : '* ?';
			$name .= $objSpielerregister->death ? ($objSpielerregister->deathday ? '; &dagger; '.\Samson\Helper::getDate($objSpielerregister->deathday) : '') : '';
			if($objSpielerregister->birthday || $objSpielerregister->death) $name .= ']';
